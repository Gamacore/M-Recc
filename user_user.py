...
def get_neighbors(user_id, user2movie, movie2user, usermovie2rating, N, neighbors, averages, deviations, K=25, limit=5):

    # find the movies the user has watched
    movies_i = user2movie[user_id]
    movies_i_set = set(movies_i)

    # calculate the average and deviation
    ratings_i = {movie:usermovie2rating[(user_id, movie)] for movie in movies_i} # user's ratings for movies
    avg_i = np.mean(list(ratings_i.values())) # user's average rating
    dev_i = {movie:(rating - avg_i) for movie, rating in ratings_i.items()} # user's deviation from average
    ...
    for neighbor_id in range(N):
        if neighbor_id != user_id:
            movies_neighbor_id = user2movie[neighbor_id]
            movies_neighbor_id_set = set(movies_neighbor_id)
            common_movies = (movies_i_set & movies_neighbor_id_set) # intersection of the two sets
            if len(common_movies) > limit:
                # calculate the average and deviation
                ratings_neighbor_id = {movie:usermovie2rating[(neighbor_id, movie)] for movie in movies_neighbor_id}
                avg_neighbor_id = np.mean(list(ratings_neighbor_id.values()))
                dev_neighbor_id = {movie:(rating - avg_neighbor_id) for movie, rating in ratings_neighbor_id.items()}
...
def save_model_data(neighbors, averages, deviations, path='./big_data/saved_user_user_model_data.pkl'):
    import pickle
    data = {
        'neighbors': neighbors,
        'averages': averages,
...
def main():
    ...
    # get the neighbors
    for i in range(N):
        get_neighbors(user_id=i, user2movie=user2movie, movie2user=movie2user, usermovie2rating=usermovie2rating, N=N, neighbors=neighbors, averages=averages, deviations=deviations)    
        if i % 1 == 0:
            print(i)
    ...
    save_all(
        ...
        train_mse,
        test_mse,
        path=f'{BIG_DATA_DIR}/all_user_user_results.pkl'
    )
